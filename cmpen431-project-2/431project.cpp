...
void printUsage () {
  fprintf(stderr,"Wrong number of arguments! Run as './DSE energy' or"
...
int main(int argc, char** argv){
  ...
  logfile << calculategeomeanEDP(GLOB_baseline)/GLOB_baseline_EP_pair.first << "," 
          << calculategeomeanExecutionTime(GLOB_baseline)/GLOB_baseline_EP_pair.second << ","
  ...
  double bestEDP = std::numeric_limits<double>::max();
  ...
  for(unsigned int iter = 0; iter < 1000; ++iter){
    ...
    double geomeanExecTimeNorm = proposedGeoTime/GLOB_baseline_EP_pair.second;

    logfile << geomeanEDPNorm << "," 
            << geomeanExecTimeNorm << ","
            << proposedGeoEDP << "," 
            << proposedGeoTime << std::endl;
    ...
    std::cout << std::endl << "             "
              << "proposedGeoEDP=" << proposedGeoEDP
              << ", bestEDP=" << bestEDP 
              << ", proposedGeoTime=" << proposedGeoTime 
              << ", bestTime=" << bestTime;
  ...
  bestfile << bestEDPconfig << ","
           ...
           << calculategeomeanExecutionTime(bestEDPconfig)/GLOB_baseline_EP_pair.second << ","
  ...
  bestfile << bestTimeconfig << ","
           ...
           << calculategeomeanExecutionTime(bestTimeconfig)/GLOB_baseline_EP_pair.second << ","
  ...
  for (int i=0; i<5; ++i){
    bestfile << calculateExecutionTime(bestTimeconfig,GLOB_prefixes[i]) << ","
             << calculateExecutionTime(bestTimeconfig,GLOB_prefixes[i])/calculateExecutionTime(GLOB_baseline,GLOB_prefixes[i]) << ",";
  }
  bestfile << std::endl;
